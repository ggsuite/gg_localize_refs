...
void main() {
  group('ManifestCommandSupport', () {
    ...
    group('bufferTsconfigWorkspaceEdit()', () {
      test('queues a write edit as a content change', () {
        final workspace = createWorkspace('manifest_support_tsconfig_write');
        final projectDir = Directory(p.join(workspace.path, 'project'))
          ..createSync(recursive: true);

        final buffer = FileChangesBuffer();
        support.bufferTsconfigWorkspaceEdit(
          projectDir: projectDir,
          edit: const PubspecOverridesEdit.write('{}\n'),
          fileChangesBuffer: buffer,
        );
        ...
        expect(
          buffer.files.single.file.path,
          p.join(projectDir.path, 'tsconfig.workspace.json'),
        );
        expect(buffer.files.single.content, '{}\n');
      });

      test('queues a delete edit as a deletion', () {
        final workspace = createWorkspace('manifest_support_tsconfig_delete');
        final projectDir = Directory(p.join(workspace.path, 'project'))
          ..createSync(recursive: true);
      ...
      test('queues nothing for an unchanged edit', () {
        final workspace = createWorkspace('manifest_support_tsconfig_noop');
        final projectDir = Directory(p.join(workspace.path, 'project'))
          ..createSync(recursive: true);
    ...
    group('bufferTsconfigWorkspaceRemoval()', () {
      test('queues the emptied paths for a dependency of the node', () {
        final workspace = createWorkspace('manifest_support_remove_paths');
        final projectDir = Directory(p.join(workspace.path, 'project'))
          ..createSync(recursive: true);
        final dependencyDir = Directory(p.join(workspace.path, 'dep'))
          ..createSync(recursive: true);
        File(p.join(dependencyDir.path, 'package.json'))
            .writeAsStringSync('{"name":"dep"}');
        File(p.join(projectDir.path, 'tsconfig.json'))
            .writeAsStringSync('{"extends": "./tsconfig.workspace.json"}');
        File(p.join(projectDir.path, 'tsconfig.workspace.json'))
            .writeAsStringSync(
              '{"compilerOptions":{"paths":{"dep":["../dep/src/index.ts"]}}}',
            );
        ...
        final buffer = FileChangesBuffer();
        final edit = support.bufferTsconfigWorkspaceRemoval(
          node: node,
          fileChangesBuffer: buffer,
        );
        ...
        expect(buffer.deletions, isEmpty);
        expect(buffer.files, hasLength(1));
        expect(buffer.files.single.content, isNot(contains('dep')));
      });

      test('queues nothing when there is no file', () {
        final workspace = createWorkspace('manifest_support_remove_no_paths');
        final projectDir = Directory(p.join(workspace.path, 'project'))
          ..createSync(recursive: true);

        final buffer = FileChangesBuffer();
        support.bufferTsconfigWorkspaceRemoval(
          node: createNode(
            name: 'project',
            directory: projectDir,
