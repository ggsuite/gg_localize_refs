...
void main() {
  group('MultiLanguageGraph', () {
    group('buildGraph', () {
      test(
        'throws when a circular dependency exists in the workspace',
        () async {
      ...
      test('throws when duplicate package '
          'names are found in the workspace', () async {
        ...
        ).writeAsStringSync('name: dup\nversion: 1.0.0\n');
        File(
          join(project2.path, 'pubspec.yaml'),
        ).writeAsStringSync('name: dup\nversion: 1.0.0\n');
