...
- **git feature branch** — `dependency_overrides` with `git: {url, ref}` in `pubspec_overrides.yaml`; `pubspec.yaml` is **not** touched either. The file is committed, so every checkout of the ticket branch resolves against the same feature branch,
- **pub.dev** — published version constraints in `pubspec.yaml`.

It supports both Dart projects (`pubspec.yaml`) and TypeScript/JavaScript projects (`package.json`) side-by-side in the same workspace. **pnpm-managed** TypeScript projects mirror the Dart architecture: `package.json` keeps its published constraints, and the `link:` specs (local mode) or `git+…#ref` pins (git mode) go into the `overrides` section of `pnpm-workspace.yaml` (`lib/src/backend/pnpm_workspace_io.dart`). Only a TypeScript project **not** managed by pnpm (no `packageManager: pnpm@…`, no `pnpm-lock.yaml`, no `pnpm-workspace.yaml`) keeps the legacy scheme: `link:` specs written straight into `package.json`, original specs backed up in `.gg/gg_localize_refs_backup_ts.json`. The reason is measured, not stylistic: npm's own top-level `overrides` field errors on any override that conflicts with a direct dependency (`EOVERRIDE` — and redirecting direct deps is exactly what localizing does), pnpm ignores that field entirely, and pnpm ≥ 11 also stopped reading `pnpm.overrides` from `package.json` — `pnpm-workspace.yaml` is the one location every pinned pnpm version honors (verified against pnpm 10.6.2 and 11.10.0, with `link:` and `git+…#branch` values, for direct deps and devDependencies).
...
| git feature branch | version constraints, untouched | `dependency_overrides` with `git: {url, ref}`, committed | untouched                         |
| pub.dev            | version constraints            | absent                                                   | restored via `restore-publish-to` |

The pnpm TypeScript analog — `package.json` plays the role of `pubspec.yaml`, the `overrides` section of `pnpm-workspace.yaml` the role of the overrides file:

| Mode               | `package.json`                 | `pnpm-workspace.yaml`                            |
| ------------------ | ------------------------------ | ------------------------------------------------ |
| local              | version constraints, untouched | `overrides` with `link:./.gg/ts_links/<name>` (shim → sibling sources) or `link:../<sibling>`, committed |
...
In local mode the `link:` of a pnpm project does not point at the sibling checkout itself but at a **shim** in `.gg/ts_links/<name>/` (`lib/src/backend/ts_link_shims.dart`): a `package.json` whose `main`/`types` are `./src/index.ts` next to a `src` symlink into the sibling. Reason: through a direct `link:` the consumer still enters the sibling via the `main`/`types` of its `package.json`, i.e. its compiled `dist/` — missing in a fresh checkout, stale after every edit, no source maps — so a breakpoint in the sibling's TypeScript never hits. Via the shim, vitest, `tsc` and the editor resolve the sibling at the source level, with no configuration in either repo. The symlink (not a relative `main` escaping the shim) is essential: TypeScript resolves `main`/`types` relative to the path inside `node_modules`, Vite relative to the real location of the shim — `./src/index.ts` means the same to both. Only a sibling with a `src/index.ts` gets a shim, everything else keeps the direct `link:`. The shims are machine-local (gitignored `.gg/`) and rewritten by every localizing run; `PnpmWorkspaceIo` treats a `link:` into `.gg/ts_links/<name>` as owned exactly like a sibling link, so the overrides are removed together with the shims by `change-refs-to-pub-dev` / `change-refs-to-git-feature-branch`. (4.5.0 tried `paths` in a `tsconfig.workspace.json` instead — that needed every repo to extend the file, which is why it was replaced.)
...
- Only `change-refs-to-pub-dev` removes the overrides — it is the one command that leaves the overrides file behind entirely. The other two _replace_ the entries of their workspace dependencies, which is what switches the mode: a `path:` written over a `git:` and back. The removal happens **before** the "nothing to do" early-out, because a repo in local or git mode has a pristine `pubspec.yaml` and a check on the manifest alone would leave the overrides shadowing the restored refs.
- Removal is structural, because pub accepts only `dependency_overrides`, `resolution` and `workspace` in that file — an ownership marker key is not possible. An entry is dropped when it is a bare `path:` pointing at a sibling checkout whose package name matches the key (`PubspecOverridesIo.isOwnedPathOverride`), or a `git:` holding nothing but a `url` and a `ref` **and** naming a workspace dependency (`isOwnedGitOverride`; the extra condition is what keeps a hand written git ref for a foreign package alive — its shape alone is not distinguishable). Hand written entries and `resolution:`/`workspace:` survive; the file is deleted only when nothing but an empty section remains.
- Pub honors `dependency_overrides` only from the **root** package of a resolution — an override a dependency declares is ignored. The two localizing commands therefore write an entry for every **transitive** workspace dependency (`ProjectNode.transitiveDependencies`), not only for the ones `pubspec.yaml` names, and `change-refs-to-pub-dev` removes that same set again. Without it a project two steps down is resolved from the registry while its siblings come from the ticket — invisible until the local version stops being API-compatible with the published one. The same rule holds for pnpm: its overrides are read from the root project only, so the pnpm TypeScript path writes an `overrides` entry per transitive workspace dependency as well. Only the legacy (non-pnpm) TypeScript path is limited to the dependencies `package.json` really names — there the specs go into the real dependency map, where extra entries would change the package's declared dependency set.
...
- `pubspec_overrides.yaml` must **not** be gitignored, so `change-refs-to-local` and `change-refs-to-git-feature-branch` _remove_ such an entry (anchored or not) instead of adding one. Reasons, all measured: the file holds only relative paths and git refs, so committing it is what makes a shared ticket workspace resolve against the sibling checkouts (local mode) or the feature branch (git mode) — the same guarantee the committed `path:` refs used to give; pub excludes the package-root `pubspec_overrides.yaml` from `dart pub publish` unconditionally (verified with the file git-tracked and no `.gitignore` present: 0 warnings), so committing it cannot leak into a release; and gitignored **and** checked in is the one combination that makes publishing fail with "checked-in files are ignored by a `.gitignore`" (exit 65). Several repositories of the suite still carry the entry by hand — running `change-refs-to-local` there clears it.
...
- `path:` refs and version-less git refs are restored from `.gg/gg_localize_refs_backup.json` — **verbatim**, via `ChangeRefsToPubDev.restoreDartRefs(reconstructGitRefs: false)`. The networkless pub.dev reconstruction must not run here: `IsOnPubDev` reads the _dependency's_ `publish_to`, which the old localization set to `none` for every repo of a ticket, so every dependency would be rewritten into a git ref with an invented `tag_pattern` — and a checkout without an `origin` remote would abort the command.
- Without a dependency backup nothing is guessed: the manifest is left alone and the user is warned.
- `publish_to: none` is reverted only when `.gg/gg_localize_refs_publish_to_backup.json` says it was not the original value. Without that backup it is kept (it cannot be told apart from a package that is private by intention) and the user is warned. The backup file is **not** deleted — `backup-publish-to`/`restore-publish-to` are driven by `gg do add`/`gg do publish` and rely on it.
- No command writes `.gg/gg_localize_refs_backup_dart.json` any more: nothing rewrites the Dart dependencies of `pubspec.yaml`, so there is nothing to remember. It is read-only legacy input for the migration above.

The pnpm TypeScript path migrates the same way: a `package.json` still carrying `link:`/`file:`/`git+` specs from the in-manifest era is restored from `.gg/gg_localize_refs_backup_ts.json` — **verbatim**, via `ChangeRefsToPubDev.restoreTypeScriptRefs(rebuildRemoteSpecs: false)`, for the same no-network reason — before the redirection is written into `pnpm-workspace.yaml`; both localizing commands do this. Without the backup the manifest is left alone with a warning, but the overrides are still written so the workspace resolves. The `_ts` backup keeps being **written** by the legacy (non-pnpm) path, which still rewrites the manifest and needs it to get back.
...
`lib/src/backend/pnpm_workspace_io.dart` is its TypeScript twin for the `overrides` section of `pnpm-workspace.yaml` (values are plain npm spec strings, not maps). It reuses `PubspecOverridesEdit` and follows the same rules — structural ownership (`link:` to a sibling whose `package.json` name matches the key; a git spec only together with a workspace dependency name), dead-link pruning, merge-don't-rewrite. Two deliberate differences: the file may pre-exist as the user's pnpm settings file, so deletion additionally requires the gg header comment that only a file created by this tool carries; and `isPnpmManaged` (packageManager field / `pnpm-lock.yaml` / `pnpm-workspace.yaml`) decides whether a TypeScript repo takes this path at all — everything else falls back to the legacy in-manifest rewrite. `hasLocalizedRefs` is the static guard query gg_multi's merge-only mode uses (»does this repo still redirect a dependency to a working copy?«), the sibling of gg_one's `NoPubspecOverrides.hasLocalizedRefs` for Dart.
...
2. Resolves the workspace root (`workspaceRootOf`): normally the **parent** of the project. A gg workspace may group its repositories in a folder named after their organization (`<workspace>/<org>/<repo>`) — then the **grandparent** is the root, recognized by the marker every gg workspace carries (the ocean by its `.ocean` folder name — or the legacy `.master`, still recognized for workspaces the gg tool has not auto-renamed yet — a ticket workspace by its `ticket.json` file). A plain folder of sibling checkouts outside a gg workspace keeps resolving to the parent.
3. Collects the candidate directories (`projectCandidateDirs`): the immediate subdirectories of the workspace root plus the children of each of them that is a _grouping folder_ — a visible directory that is neither a project of any registered language nor a git repository. An organization folder is one, a repository is not, so projects nested inside a repository (`example/`, fixtures) stay invisible. For each candidate that `language.isProjectRoot` recognizes, `createNode` is called. A single `buildGraph` pass covers one language; duplicate names throw.
...
TypeScript scenarios under `test/sample_folder_ts/localize_refs/`: `pnpm_succeed` (plain pnpm project, sibling without sources → direct `link:`), `pnpm_with_sources` (the sibling has a `src/index.ts` — drives the shim tests of all three commands).
